...
def clean_expired_search(db_connection, table, archive):
    """Переносит в архив запросы, созданные более суток назад и удаляет их из поиска"""
    ...
    cursor.execute(
        ...
        "WHERE EXTRACT(EPOCH FROM now() - search_init_date)/3600 > 24;",
        {"archive": AsIs(archive), "table_name": AsIs(table)},
    )
    cursor.execute(
        "DELETE FROM %(table_name)s WHERE EXTRACT(EPOCH FROM now() - search_init_date)/3600 > 24;",
